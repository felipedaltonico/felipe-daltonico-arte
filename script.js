...
function artworkName(src, index) {
  try {
    ...
    const filename = decodeURIComponent(url.pathname.split("/").pop() || `arte-${index + 1}`);
    return filename.replace(/\.[^.]+$/, "").replace(/[-_]+/g, " ");
  } catch {
    const filename = decodeURIComponent(String(src).split("/").pop() || `arte-${index + 1}`);
    return filename.replace(/\.[^.]+$/, "").replace(/[-_]+/g, " ");
  }
}
...
function renderPortfolio(list) {
  ...
  images.forEach((src, index) => {
    ...
    const name = artworkName(src, index);
    figure.setAttribute("aria-label", `Abrir ${name} em tela cheia`);
    ...
    // Se uma imagem específica falhar, ela some sem deixar ícone quebrado.
...
async function githubFolderImages(path) {
  const url = `https://api.github.com/repos/felipedaltonico/felipe-daltonico-arte/contents/${path}?ref=main`;
...
async function loadPortfolioImages() {
  const localList = Array.isArray(window.PORTFOLIO_IMAGES)
    ? window.PORTFOLIO_IMAGES
    : [];

  // No GitHub Pages, buscamos a pasta real do repositório.
  if (location.hostname === "felipedaltonico.github.io") {
    try {
      const fromPortfolio = await githubFolderImages("portfolio");

      if (fromPortfolio.length) {
        return fromPortfolio;
      }

      // Fallback: caso as imagens tenham sido enviadas para a raiz por engano.
      const fromRoot = await githubFolderImages("");
      if (fromRoot.length) {
        return fromRoot;
      }
    } catch (error) {
      console.warn("Não foi possível carregar o portfólio pelo GitHub:", error);
    }
  }

  // Abrindo localmente continua usando portfolio.js.
  return localList;
}

loadPortfolioImages().then(renderPortfolio);
